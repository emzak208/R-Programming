...
#        Note the value of x[1] is 4, so the expression x[1] > 6 evaluates to FALSE.
...
#  Code is easier to read when if clauses and else clauses are indented to the same level.
...
as.logical('a')  # no clear way to converta character to alogical value


##  The as.logical function converts nonzero numeric values to TRUE
##  and zero values to FALSE. There are many as.<something> functions
##  in R. Their purpose is to convert the argument to be <something>.
##  For example as.numeric converts its input argument to a numeric
##  quantity.
as.logical(as.numeric('0'))

##  Let's see what as.<something> functions R has

all.as   = apropos('as')
as.start = grep('^as',all.as)
all.as[as.start]

##   In this example x[2] is NOT converted (or cast in C terminology) to a 
##   logical value before doing the comparison.
##   Instead TRUE is converted to 1. Since x[2] does not equal 1, the
...
##  the expession inside the parentheses follwing "if" will be evaluates to a 
##  logical variable. x[2] is converted to TRUE.  
{
if ( x[2]  )
  {
    print('if clause')
...
# One of the most insidious errors is using "=" instead of "==" in an if statement.
# This is a problem R has inherited from C. If R insisted on using "<-" as the 
# assignment operator and did not allow "=" for the assignment operator, 
# then we could use "=" as the equality relational operator and our lives would be 
# easier.
# The first modern programming language was Algol 60. It used ':=' as the assignment
# operator, and used '=' inside the if statement. This is what all descendents of
# Algol did until C came along.
...
# a complete syntax unit. The if clause is not executed.
...
##  Recall the warning said only the first condition is used.

if ( (x & y)[-1] ) print('if clause') else print('else clause')

##  Here we use the deselection operator to get rid of the first
##  condition, so the second condition becomes the first.
...
#   be true. We do not evaluate b. Simlar considerations apply to "||". If we
...
####   This is very interesting. One expects that always one clause to executes.
...
##  Recall that a vector is supposed to have all elements of the same type.
...
x[4] == NA
NA   == NA      # cannot use '==' to see if two variables are both NA

is.na(NA)
is.na(x[4])
is.na(x)
sum(is.na(x)) > 0  # this can detect if any elements of x have NA value
...
##    At this point one wonders if one is in an Alice in Wonderland world.
##    NA is logical (we are told), and vector elements must have identical type.
##    But an NA inside a vector is numeric.

sqrt(-1)

##    Note we get NaN as the answer. This means "not a number". In the real number
##    system the square root of -1 does not exist.

a = sqrt(-1)
mode(a)
storage.mode(a)
is.na(a)                  # is.na thinks an NaN is an NA. This is not good.
is.nan(a)
is.nan(y)
is.nan(z)

sum(x)                     # since x[4] is NA we csannot calculate a sum
sum(x,na.rm=TRUE)          # na.rm = TRUE says to remove NA values when we
                           # calculate the sum. The vector x is unchanged.
                           # na.ignore would be a more accurate description.
x

t = x                      
if ( sum(is.na(x)) > 0 )   # This code removes NA (and NaN) values from x
  {
    t = x[-which(is.na(x))]        
  }
print(t)
...
sum(xx,na.rm=TRUE)         # na.rm also skips NaN

##     Unfortunately some R functions treat NaN values differently than NA
##     values, and some treat them identically. You do not know until you
##     try the function.

x  + xx  # Note that NA + NaN gives NA
xx + x   # Note that NaN + NA gives NaN
...
2 ^ 3 ^ 4  # order of operations is important and not always what one might think
...
2^5000000
#### The biggest number could be type:
.Machine
