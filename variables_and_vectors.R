...
x2 = c(5)          # has the same effect as x2 = 5
print(x2)          # c() turns something into a vector 
...
x = c(10,v) #  add 10 in front of vector v
x
...
n = 7;
v[3:n+1] # order of operation ':' comes first
...
s = c(1,5,7,8,11)
v[s]      # we can select elements based on any subset
...
# You can always use parentheses to reverse the natural operator precedence.
#  3*(2+5) = 3*7 = 21, but 3*2+5 equals 6+5 = 11.

# It is usually best to use parentheses when you are unsure of precedence. Even if you are correct,
